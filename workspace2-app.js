...
async function createCorrelation(sourceOverride = null) {
  if (byId("correlation-analysis").value === "structured") {
    invalidateCorrelation();
    setStatus("Calculating structured correlation methods...");
    await yieldToBrowser();
    const sourceRows = sourceOverride?.rows ?? rowsForAnalysis("correlation-data-scope");
    ...
    if (sourceOverride?.metadata) result.results.forEach(row => { row.status = `${row.status || ""}; Source exclusion: ${sourceOverride.metadata.label}, ${sourceOverride.metadata.axis} ${sourceOverride.metadata.side}, ${sourceOverride.metadata.excludedPairs}/${sourceOverride.metadata.initialPairs} pairs`; });
    state.lastStructuredCorrelation = { ...result, source: state.source, scope: byId("correlation-data-scope").value,
      plotHeaders: state.headers.slice(), plotRows: sourceRows.map(row => row.slice()), plotParameters: state.structuredParameters.slice(), plot: null, sourceBaseRows: sourceOverride?.baseRows, sourceExclusion: sourceOverride?.metadata };
...
// Workspace 2-only source screening. The original application's entry point is unchanged.
export function workspace2SourceOutlierState() {
  const current = state.lastStructuredCorrelation;
  return { active: Boolean(current?.sourceExclusion), hasPlotSteps: Boolean(current?.plot?.trimHistory?.length), ...(current?.sourceExclusion || {}) };
}
export async function applyWorkspace2SourceOutliers(options) {
  const current = state.lastStructuredCorrelation, selected = current?.originalPlot;
  if (!selected) throw new Error('Run structured correlation and open a plot first.');
  const { filterSourceOutliers } = await import('./workspace2/source-outliers.js?v=2');
  if (state.lastStructuredCorrelation !== current) throw new Error('The dataset changed. Run correlation again.');
  const baseRows = (current.sourceBaseRows || current.plotRows).map(row => row.slice());
  const screened = filterSourceOutliers(current.plotHeaders, baseRows, selected.xParameter, selected.yParameter, options);
  await createCorrelation({ rows: screened.rows, baseRows, metadata: screened.metadata });
  const rebuilt = state.lastStructuredCorrelation;
  const index = rebuilt.results.findIndex(row => row.method === selected.method && row.xParameter === selected.xParameter && row.yParameter === selected.yParameter);
  if (index >= 0 && rebuilt.results[index].r !== null) plotStructuredCorrelation(index);
  setStatus(`${screened.metadata.label}: ${screened.metadata.excludedPairs}/${screened.metadata.initialPairs} source pairs removed (${screened.metadata.excludedPercent.toFixed(3)}%). Correlation table and plot recalculated.`, false, true);
}
export async function resetWorkspace2SourceOutliers() {
  const selected = state.lastStructuredCorrelation?.originalPlot;
  await createCorrelation();
  const current = state.lastStructuredCorrelation;
  const index = current.results.findIndex(row => row.method === selected?.method && row.xParameter === selected?.xParameter && row.yParameter === selected?.yParameter);
  if (index >= 0 && current.results[index].r !== null) plotStructuredCorrelation(index);
  setStatus('Source exclusions reset. Original correlation table and plot restored.', false, true);
}
