...
async function parseWorkbook(data, fileName) {
  ...
  state.dataLabels = [];
  lastComplaintImport=null; complaintMachineTable=null;
...
function renderDataLabelsTable() {
  ...
  if (document.getElementById("complaint-label-import")) { renderLabelTableView(container,state.dataLabels.map(item=>{
    const matches=dataRows().filter(row=>sameDataValue(row[headerIndex(state.headers,"Lot")],item.lot) && sameDataValue(row[headerIndex(state.headers,"N")],item.batch));
    const values=Object.fromEntries(item.parameters.map(parameter=>{
      const column=zoneColumns(state.headers,parameter)?.[item.zone-1] ?? -1;
      return [parameter,matches.length===1 && column>=0 ? matches[0][column] : item.values?.[parameter]];
    }));
    return {...item,values};
  })); return; }
  container.innerHTML = renderTable([
    ["Lot", "MR N", "Zone", "Complaint reason / Label", "Parameters", "Values", "Comment", "Notes", "Updated"],
...
let lastComplaintImport = null;
let complaintMachineTable=null;
function complaintMachines() {
  if (complaintMachineTable?.bytes===state.originalData) return complaintMachineTable.rows;
  let rows=[];
  try {rows=readAuswertungTable(state.originalData);} catch { /* Clean-only workbooks require machine confirmation. */ }
  complaintMachineTable={bytes:state.originalData,rows}; return rows;
}
function refreshProblemLabels() {
  renderDataLabelsTable(); renderPreviewTable(); syncLabelEditor();
}
function applyComplaintLabels(labels) {
  if (!state.workbook) throw new Error("Open an IPW workbook first.");
  const previous=state.dataLabels;
  const merged=mergeProblemLabels(previous,labels);
  if (merged.added) {
    state.dataLabels=merged.labels;
    lastComplaintImport={previous,applied:state.dataLabels};
    refreshProblemLabels();
  }
  return merged.added;
}
function undoComplaintLabels() {
  if (!lastComplaintImport || state.dataLabels!==lastComplaintImport.applied) throw new Error("No unchanged import batch is available to undo.");
  state.dataLabels=lastComplaintImport.previous; lastComplaintImport=null; refreshProblemLabels();
}
async function exportComplaintResults(review,retainChecks=[]) {
  const XLSX=getXlsx();
...
async function saveComplaintLabels() {
  try {const mode=await saveLabelsToCurrentWorkbook({downloadCopy:true}); setStatus('Labeled IPW copy downloaded. The source file was not replaced.',false,true); return mode;}
...
async function saveLabelsToCurrentWorkbook(options = {}) {
  const fileName = options.downloadCopy ? writableWorkbookName().replace(/(\.[^.]+)$/, '_complaint_labels$1') : writableWorkbookName();
  const saveHandle = options.downloadCopy ? null : await requestWorkbookSaveHandle(fileName);
  ...
  let saveMode;
  if(options.downloadCopy) {downloadBlob(fileName,new Blob([bytes],{type:workbookMime(fileName)})); saveMode='download';}
  else saveMode = await saveWorkbookBytes(bytes, fileName, saveHandle);
  state.originalData = bytes.slice(0);
  return saveMode;
}
...
function renderCurrentData() {
  ...
  if (document.querySelector('.complaint-context')) {
    window.dispatchEvent(new CustomEvent('ipw-investigation-dataset', { detail: {
      ...
      locate: (row, column) => {
        const address = state.sourceLocations.get(row);
        const sourceColumn = state.sourceColumns[column];
        return { row: address?.row ?? null, cell: address && sourceColumn !== null && sourceColumn !== undefined ? getXlsx().utils.encode_cell({ r: address.row - 1, c: sourceColumn }) : '' };
      },
      isError: (row, column) => {
        const address = state.sourceLocations.get(row), c = state.sourceColumns[column];
        if (!address || c === null || c === undefined) return false;
        return state.workbook?.Sheets[state.source]?.[getXlsx().utils.encode_cell({ r: address.row - 1, c })]?.t === 'e';
      }
    } }));
  }
  syncGaussianSourceChoices();
...
function gaussianLotsForSource(sourceId) {
  const lotColumn=headerIndex(state.headers,'Lot');if(lotColumn<0)return [];
  const lots=new Set();
  for(const row of dataRows()){
    ...
    const lot=text(row[lotColumn]);if(lot)lots.add(lot);
  }
...
function gaussianRows(lotOverride='') {
  ...
  const lot=lotOverride || byId('gaussian-lot').value,lotColumn=headerIndex(state.headers,'Lot');
  return lotColumn<0?[]:dataRows().filter(row=>sameDataValue(row[lotColumn],lot));
}

function collectGaussianRecords(parameter, includedZones, sourceId = 'all',lotOverride='') {
  ...
  for (const row of gaussianRows(lotOverride)) {
...
function createAssessment() {
  ...
  const compareLot = byId("assessment-compare-lot").value;
  if (compareLot) {
    state.lastAssessment.secondaryLot = compareLot;
...
function updateSecondaryAssessment(parameter) {
  ...
  const settingsLotSelect=byId('assessment-compare-lot');if(settingsLotSelect)settingsLotSelect.value='';
...
function updateSecondaryLotAssessment(lot) {
  ...
  const settingsLotSelect=byId('assessment-compare-lot');if(settingsLotSelect)settingsLotSelect.value=lot;
...
function syncAssessmentCompareLots() {
  const selectedLot = byId("assessment-lot").value;
  const control = byId("assessment-compare-lot");
  const lots = state.lots.filter((lot) => !sameDataValue(lot, selectedLot));
  const previous = control.value;
  fillSelect(control, ["", ...lots], lots.some((lot) => sameDataValue(lot, previous)) ? previous : "", (value) => value || "One Lot");
  control.disabled = !lots.length;
}
