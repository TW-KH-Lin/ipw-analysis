const CACHE_NAME = "ipw-workspace-v16";
const APP_SHELL = [
  "./index.html",
  "./workspace.css?v=1",
  "./workspace-ui.js?v=4",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => key.startsWith("ipw-workspace-") && key !== CACHE_NAME).map((key) => caches.delete(key))))
